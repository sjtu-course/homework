...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	if (flag)
	{
		...
		in >> s;
		int  pos = s.find("|");
		userName.assign(s, 0, pos);
		roomName.assign(s.begin() + pos, s.end());
		in.close();
...
void CRTCDemoDlg::WriteProfile(CString& name, CString& room)
{
	std::ofstream os("INI");
	os << name << " " << room;
	os.close();
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	WriteProfile(user_name, room_name);
	//WriteProfileString(L"Test", L"NAME", user_name);
	//WriteProfileString(L"Test", L"ROOM", room_name);	
