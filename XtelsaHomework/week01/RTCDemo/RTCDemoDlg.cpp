...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	GetModuleFileName(NULL, modulePath, MAX_PATH);//null为空，表示当前模块
	(_tcsrchr(modulePath, _T('\\')))[0] = 0;//_tcsrchr返回的是最后一次出现查找字符的指针，将这个指针赋值为0，也就是将其删除，后面得到的modulepath就删掉了文件名
	//设置读取路径
	CString iniSavePath = (CString)modulePath + "\\loginInfo.ini";

	GetPrivateProfileString(_T("LoginInfo"), _T("user_name"), user_name, user_name.GetBufferSetLength(MAX_PATH), MAX_PATH, iniSavePath);//https://blog.csdn.net/qqq1994_0810/article/details/82686622
	GetPrivateProfileString(_T("LoginInfo"), _T("room_name"), room_name, room_name.GetBufferSetLength(MAX_PATH), MAX_PATH, iniSavePath);//https://blog.csdn.net/qqq1994_0810/article/details/82686622
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	CString iniSavePath = (CString)modulePath + "\\loginInfo.ini";
	//保存配置
	WritePrivateProfileString(_T("LoginInfo"), _T("user_name"), user_name, iniSavePath);
	WritePrivateProfileString(_T("LoginInfo"), _T("room_name"), room_name, iniSavePath);
