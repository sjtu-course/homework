...
- (void)viewDidLoad {
    ...
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];  //定义自动填充用户信息
    ...
    if ([userDefaultes objectForKey:@"ROOM"]) {
        roomInput.text = [userDefaultes objectForKey:@"ROOM"];
    }
    else{
    ...
    if ([userDefaultes objectForKey:@"Name"]) {
        nameInput.text = [userDefaultes objectForKey:@"Name"];
    }
    else{
...
- (void)enterButtonTapped:(id)sender {
    ...
    [userDefaults setObject:_roomInput.text forKey:@"ROOM"];
    [userDefaults setObject:_nameInput.text forKey:@"Name"];
