...
{	
	...
	CRTCDemoDlg::InitName();   //<Jixiang: read from ini

	return TRUE;  // return TRUE  unless you set the focus to a control
}

//Jixiang:
void CRTCDemoDlg::InitName()
{
	CString path_name;
	CChatDialog chat_dlg;
	path_name = chat_dlg.GetExePath();
	//OutputDebugString(path_name);
	CString strSection = _T("Section1");
	CString strSectionKey = _T("Room");
	TCHAR strBuff[256];
	GetPrivateProfileString(strSection, strSectionKey, NULL, strBuff, 80, path_name);
	m_room.SetWindowText(CString(strBuff));
	strSectionKey = _T("User");
	GetPrivateProfileString(strSection, strSectionKey, NULL, strBuff, 80, path_name);
	m_name.SetWindowText(CString(strBuff));
}
...
{
	CString user_name, room_name, path_name;
	m_name.GetWindowText(user_name);
	m_room.GetWindowText(room_name);

	CRTCDemoDlg::ClickedLoginCached(user_name, room_name); //Jixiang: write
...
//Jixiang:----------*********
void CRTCDemoDlg::ClickedLoginCached(const CString &user_name, const CString& room_name)
{
	CString path_name;
	CChatDialog chat_dlg;
	path_name = chat_dlg.GetExePath();
	...
	WritePrivateProfileString(strSection, strSectionKey, user_name, path_name);
	strSectionKey = _T("Room");
	WritePrivateProfileString(strSection, strSectionKey, room_name, path_name);
}
