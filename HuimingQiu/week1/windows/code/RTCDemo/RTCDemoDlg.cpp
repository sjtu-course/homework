...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	CString room;
	CString user;
	readRoomAndUser(room, user);
	this->m_room.SetWindowTextW(room);
	this->m_name.SetWindowTextW(user);
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	if (room_name.GetLength() == 0) {
		AfxMessageBox(L"Error: empty room! ");
		return;
	}
	if (user_name.GetLength() == 0) {
		AfxMessageBox(L"Error: empty user! ");
		return;
	}

	writeRoomAndUser(room_name, user_name);
