...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	// MFC中ASSERT作为断言语句
	// 括号内内容为TRUE，继续执行；为FALSE终止执行。
	// 之后取得当前窗口的系统菜单，在这个菜单中添加字符串资源IDS_ABOUTBOX和菜单资源IDM_ABOUTBOX。
	...
	// 获取上次的登录信息
	GetPrivateProfileStringW(_T("Register_Info"), _T("Room"), _T(" "), USER, 10,PATH);
	GetPrivateProfileStringW(_T("Register_Info"), _T("Name"), _T(" "), ROOM, 10, PATH);
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	// 保存登录信息
	WritePrivateProfileStringW(_T("Register_Info"), _T("Room"), room_name,PATH);
	WritePrivateProfileStringW(_T("Register_Info"), _T("Name"), user_name, PATH);
