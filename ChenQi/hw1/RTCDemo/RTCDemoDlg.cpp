...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	CString path = L".\\Login.ini";
	
	//设置指定装载到lpReturnedString缓冲区的最大字符数量
	DWORD buffer = 1024;
	CString room_name;
	CString user_name;
	//获取内部字符指针
	TCHAR* pRoom = new TCHAR[1024];
	TCHAR* pUser = new TCHAR[1024];
	
	GetPrivateProfileString(L"Login", L"RoomName", L"", pRoom, buffer, path);
	GetPrivateProfileString(L"Login", L"UserName", L"", pUser, buffer, path);
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	LPTSTR lpPath = L".\\Login.ini";

	// CString to LPCWSTR
	LPCWSTR pUserName = user_name.AllocSysString();
	LPCWSTR pRoomName = room_name.AllocSysString();

	WritePrivateProfileString(L"Login", L"RoomName", pRoomName, lpPath);
	WritePrivateProfileString(L"Login", L"UserName", pUserName, lpPath);
