...
void CRTCDemoDlg::OnPaint()
{
	...
	// read from ini and initalize the user name and room name
	LPTSTR USER = new TCHAR[100];
	LPTSTR ROOM = new TCHAR[100];
	LPTSTR PATH = _T("..\\RTC_Config.ini");
	GetPrivateProfileString(_T("ID"), _T("user"), _T(""), USER, 10, PATH);
	GetPrivateProfileString(_T("ID"), _T("room"), _T(""), ROOM, 10, PATH);
	m_name.SetWindowTextW(USER);
	m_room.SetWindowTextW(ROOM);
}
...
{
	// get data from window and write into ini
	LPTSTR PATH = _T("..\\RTC_Config.ini");
	CString user_name, room_name;
	m_name.GetWindowText(user_name);
	m_room.GetWindowText(room_name);
	WritePrivateProfileString(_T("ID"), _T("user"), user_name, PATH);
	WritePrivateProfileString(_T("ID"), _T("room"), room_name, PATH);
