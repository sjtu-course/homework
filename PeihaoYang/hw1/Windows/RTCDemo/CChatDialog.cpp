	...
	: CDialogEx(IDD_CHATDIALOG, pParent)
{
	...
	m_ini.Format(L"%s\\%s", m_pwd, CONFIG_INI);
}
...
void CChatDialog::SaveRoomUser() {
	// save m_room and m_name to ini file, the return value is BOOL
	WritePrivateProfileString(L"User", L"room", m_room, m_ini);
	WritePrivateProfileString(L"User", L"name", m_name, m_ini);
}