...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	//显示上次登录的房间名和密码名
	CString sPath, room_name, user_name;
	GetModuleFileName(NULL, sPath.GetBufferSetLength(MAX_PATH+1), MAX_PATH);
	sPath = sPath.Left(sPath.ReverseFind('\\'));
	sPath += "\\user_info.ini";

	GetPrivateProfileString(L"USER", L"room_name", L"", room_name.GetBuffer(512), 512, sPath);
	GetPrivateProfileString(L"USER", L"user_name", L"", user_name.GetBuffer(512), 512, sPath);
    m_name.SetWindowText(room_name);
	m_room.SetWindowText(user_name);
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	WritePrivateProfileString(L"USER", L"room_name", room_name, sPath);
	WritePrivateProfileString(L"USER", L"user_name", user_name, sPath);
