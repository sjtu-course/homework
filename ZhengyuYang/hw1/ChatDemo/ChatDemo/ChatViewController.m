...
- (id)setUser:(NSString *)room user:(NSString *)user {
    self.title = [NSString stringWithFormat:@"房间: %@", room];
    self.user = user;
...
- (void)sendButtonTapped:(id)sender {
    if (_input.text.length > 0) {
        ...
        [_messageArray addObject:[NSString stringWithFormat:@"%@: %@", self.user, _input.text]];
        //NSLog(self.user);
        //NSLog(@"%@: %@", self.user, _input.text);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"%@", [_messageArray objectAtIndex:indexPath.row]];
    return cell;
}
@end
