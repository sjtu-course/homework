...
BOOL CRTCDemoDlg::OnInitDialog()
{
	...
	GetModuleFileName(NULL, File_Path.GetBufferSetLength(MAX_PATH), MAX_PATH);
	int nPos = File_Path.ReverseFind(_T('\\'));
	File_Path = File_Path.Left(nPos + 1);

	CString user, room;
	File_Path = File_Path + _T("init.ini");
	GetPrivateProfileString(_T("Init"), _T("InitName"), _T(""), user.GetBuffer(512), 512, File_Path);
	GetPrivateProfileString(_T("Init"), _T("InitRoom"), _T(""), room.GetBuffer(512), 512, File_Path);
...
void CRTCDemoDlg::OnBnClickedLogin()
{
	...
	WritePrivateProfileString(_T("Init"), _T("InitName"), user_name, File_Path);
	WritePrivateProfileString(_T("Init"), _T("InitRoom"), room_name, File_Path);
